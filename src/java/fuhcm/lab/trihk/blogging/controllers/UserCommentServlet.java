 ...
 * @author huynh
 */
@WebServlet(name = "UserCommentServlet", urlPatterns = {"/UserCommentServlet"})
public class UserCommentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String strId = request.getParameter("atcId");
            String path = "MainController?action=view&id=" + strId;
            int artId = Integer.parseInt(strId);
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
            LocalDateTime now = LocalDateTime.now();
            String date = formatter.format(now);
            CommentDTO dto = new CommentDTO(0, comment, userEmail, artId, date);
