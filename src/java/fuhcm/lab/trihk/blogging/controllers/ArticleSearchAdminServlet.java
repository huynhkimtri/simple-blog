 ...
 * @author huynh
 */
@WebServlet(name = "ArticleSearchAdminServlet", urlPatterns = {"/ArticleSearchAdminServlet"})
public class ArticleSearchAdminServlet extends HttpServlet {

    private final String showResultPage = "admin.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                switch (cbxStatus) {
                    case Constants.STATUS_ARTICLE_NEW:
                        statusValue = Constants.STATUS_ARTICLE_NEW;
                        break;
                    case Constants.STATUS_ARTICLE_ACTIVE:
                        statusValue = Constants.STATUS_ARTICLE_ACTIVE;
                        break;
                    case Constants.STATUS_ARTICLE_DELETE:
                        statusValue = Constants.STATUS_ARTICLE_DELETE;
                        break;
                    default:
                        statusValue = Constants.EMPTY_STRING;
                        break;
                }
                int totalResults = dao.getRowsCount(searchValue, statusValue);
                ...
                dao.getListArticleAndAuthorPagingByTitleAndStatus(searchValue, statusValue, start, stop);

                List<ArticleMapper> list = dao.getListArticlesMapper();
                request.setAttribute("LIST_SEARCH_RESULTS", list);
                request.setAttribute("SEARCH_VALUE", searchValue);
                request.setAttribute("STATUS_VALUE", cbxStatus);
